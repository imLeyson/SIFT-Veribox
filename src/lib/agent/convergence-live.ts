...
2. 提问必须直击视觉设计的分水岭判断（Design Crossroads）：
   - 视觉质感与感官锚点（例如：品质感主要依赖特种纸微纹理与低反光触感，还是依赖极端克制的网格版式？）
   - 视觉层级与第一视线焦点（例如：包装正面第一视觉重心先突出核心茶品与风味，还是先确立醒目的品牌字标？）
   - 约束冲突的视觉化解取舍（例如：预算严格受限时，是用单色特种纸压凹克制传达，还是用高对比排版骨架规避材质平庸？）
   - 明确的视觉禁忌与审美边界（例如：坚决杜绝大面积渐变色、杜绝常见模板化插画、杜绝样机贴图感）。
3. 每轮必须问 2–3 个互不重复的问题，按 blocking > material 排序；每题只对应一个判断。
4. 问题字数精炼（≤40字）；选项 2–3 个且每项最多 32 字，选项对立鲜明，反映真实设计流派与手法差异；没有真实可比选项就 options=[]。不推荐或预选答案。
5. 不重复已回答、已暂缓或上一轮语义相同的问题；本轮已有答案能推导出的判断不要再问。
6. 不问已给出的受众或约束；不强制定死具体色值 HEX 或字号 pt，重点是视觉语言的感知基调、表达主次、禁忌与评价准则。
7. 每次回答后必须重写 currentHypothesis，使它比上一轮更具体且富有画面感，建议格式：以[核心视觉语言/材质/排版结构]在[媒介与成本约束]下呈现[视觉心理感知与张力]，坚决杜绝[视觉禁忌]（最多 120 字）。
